...
Sensor.getCategories = (result) => {
  sql.query('SELECT c.id, c.category_name, COALESCE(ci.image_path, NULL) AS image_path FROM categories c LEFT JOIN category_images ci ON c.id = ci.category_id', (err, res) => {
    if (err) {
